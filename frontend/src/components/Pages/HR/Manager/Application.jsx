...
import { Eye, Edit, FileText, User, Phone, Mail, MapPin, Calendar, DollarSign, GraduationCap, Briefcase, Award, Link, MessageSquare, X, CheckCircle, XCircle, Clock, UserCheck, Users, AlertCircle } from "lucide-react";
...
const Recruitment = () => {
  ...
  const statuses = [
    { value: 'pending', label: 'Pending', color: 'text-yellow-500', bgColor: 'bg-yellow-100 dark:bg-yellow-900/20', icon: Clock },
    { value: 'under_review', label: 'Under Review', color: 'text-blue-500', bgColor: 'bg-blue-100 dark:bg-blue-900/20', icon: Eye },
    { value: 'shortlisted', label: 'Shortlisted', color: 'text-purple-500', bgColor: 'bg-purple-100 dark:bg-purple-900/20', icon: UserCheck },
    { value: 'interview_scheduled', label: 'Interview Scheduled', color: 'text-indigo-500', bgColor: 'bg-indigo-100 dark:bg-indigo-900/20', icon: Calendar },
    { value: 'hired', label: 'Hired', color: 'text-green-500', bgColor: 'bg-green-100 dark:bg-green-900/20', icon: CheckCircle },
    { value: 'rejected', label: 'Rejected', color: 'text-red-500', bgColor: 'bg-red-100 dark:bg-red-900/20', icon: XCircle }
  ];
  ...
  const handleViewApplication = async (applicationId) => {
    try {
      const res = await axios.get(`${base_Url}/api/hr/applications/${applicationId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const filteredApplications = Array.isArray(applications)
    ? applications.filter(
        (app) => {
          ...
          const matchesDepartment = !departmentFilter || app.jobId?.department === departmentFilter;
  ...
  const getStats = () => {
    ...
    const interview_scheduled = applications.filter(app => app.status === 'interview_scheduled').length;
    const rejected = applications.filter(app => app.status === 'rejected').length;
    const hired = applications.filter(app => app.status === 'hired').length;
    
    return { total, pending, underReview, shortlisted, hired ,interview_scheduled ,rejected};
  };
...
<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-6">
  ...
  <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Under Review</p>
        <p className="text-xl font-bold text-blue-600">{stats.underReview || "0"}</p>
      </div>
  ...
  <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Shortlisted</p>
  ...
  <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Interview</p>
        <p className="text-xl font-bold text-indigo-600">{stats.interview_scheduled || "0"}</p>
      </div>
              ...
              filteredApplications.map((app) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-3">
                      <div className={`flex items-center space-x-1 px-2 py-1 rounded-md  text-xs font-medium ${statusInfo.bgColor} ${statusInfo.color}`}>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center p-4 z-50">
          <div className="relative bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-gray-800 text-white p-6 rounded-t-xl">
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  <h2 className="text-2xl font-bold">{viewApplication.fullName}</h2>
                  <p className="text-blue-100">{viewApplication.jobId?.jobTitle}</p>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium text-gray-800 dark:text-gray-200">{viewApplication.jobId?.jobTitle}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Department</p>
                    <p className="font-medium text-gray-800 dark:text-gray-200">{viewApplication.jobId?.department}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Location</p>
                    <p className="font-medium text-gray-800 dark:text-gray-200">{viewApplication.jobId?.jobLocation}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Required Experience</p>
                    <p className="font-medium text-gray-800 dark:text-gray-200">{viewApplication.jobId?.experience}</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="font-medium text-gray-800 dark:text-gray-200">
                        {new Date(viewApplication.dateOfBirth).toLocaleDateString()}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Address</p>
                      <p className="font-medium text-gray-800 dark:text-gray-200">
                        {viewApplication.address?.street}, {viewApplication.address?.city}, {viewApplication.address?.state} {viewApplication.address?.zipCode}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="font-medium text-gray-800 dark:text-gray-200">{viewApplication.experience}</p>
                    </div>
                    {viewApplication.currentSalary && (
                      <div>
                        <p className="text-sm text-gray-600 dark:text-gray-400">Current Salary</p>
                        <p className="font-medium text-gray-800 dark:text-gray-200">₹{viewApplication.currentSalary.toLocaleString()}</p>
                      </div>
                    )}
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Expected Salary</p>
                      <p className="font-medium text-gray-800 dark:text-gray-200">₹{viewApplication.expectedSalary.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Notice Period</p>
                      <p className="font-medium text-gray-800 dark:text-gray-200">{viewApplication.noticePeriod}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  {viewApplication.education?.map((edu, index) => (
                    <div key={index} className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">{edu.institution} • {edu.year} • {edu.percentage}%</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Previous Experience */}
              {viewApplication.previousExperience && viewApplication.previousExperience.length > 0 && (
                <div>
                  <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-3 flex items-center">
                    <Briefcase className="w-5 h-5 mr-2" />
                    Previous Experience
                  ...
                  <div className="space-y-3">
                    {viewApplication.previousExperience.map((exp, index) => (
                      <div key={index} className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{exp.responsibilities}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Status Information */}
              <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-3">Application Status</h3>
                <div className="flex items-center space-x-4">
                  <div className={`flex items-center space-x-2 px-3 py-2 rounded-full ${getStatusInfo(viewApplication.status).bgColor} ${getStatusInfo(viewApplication.status).color}`}>
                    {React.createElement(getStatusInfo(viewApplication.status).icon, { className: "w-4 h-4" })}
                    <span className="font-medium">{getStatusInfo(viewApplication.status).label}</span>
                  </div>
          ...
          <div className="relative bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl w-full max-w-md">
            ...
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                {selectedApplication.fullName} - {selectedApplication.jobId?.jobTitle}
              </p>
            </div>
